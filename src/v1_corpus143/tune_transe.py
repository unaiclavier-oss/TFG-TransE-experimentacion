...
con el **MRR filtrado** sobre el conjunto de validación. Elige la mejor
configuración, reentrena con ella y sobrescribe los artefactos de transe_run/.

Importante: las métricas MRR/Hits@k se usan aquí SOLO como criterio interno de
selección de hiperparámetros. NO son el resultado central de §3.4 (cualitativo);
se reservan como línea cuantitativa futura, conforme al planning.
...
from train_transe import (OUTPUT_DIR, DATA_CSV, SEED, P_NORM, BATCH_SIZE,
                          load_triples, build_vocab, to_indices, split_triples,
                          TransE, train, save_split)


# ---- Rejilla de búsqueda (pequeña: grafo de 51 entidades, CPU) ----
GRID = {
    "dim":    [30, 50],
    "margin": [1.0, 2.0, 4.0],
    "lr":     [0.01],
...
def train_one(train_idx, Ne, Nr, dim, margin, lr, epochs):
    ...
    model = TransE(Ne, Nr, dim=dim, p=P_NORM)
...
def main():
    ...
    print(f"\n{len(combos)} configuraciones · selección por MRR(val) filtrado\n")
    ...
    for c in combos:
        ...
        if best is None or m["MRR"] > best["MRR"]:
            best = row

    print(f"\nMejor configuración (MRR val = {best['MRR']:.3f}):")
